...
def setup_log_rotation(*, rotate_logs: bool, config_path: Optional[str]) -> None:
    ...
    if config_path:
        if not os.path.exists(config_path):
            print(f"⚠️  Log config file not found: {config_path} (using defaults)")
        else:
            try:
                ...
                if isinstance(loaded, dict):
                    ...
                    print(f"⚠️  Log config YAML must be a mapping/dict: {config_path} (using defaults)")
            except Exception as e:
                print(f"⚠️  Failed to load log config YAML '{config_path}': {e} (using defaults)")
