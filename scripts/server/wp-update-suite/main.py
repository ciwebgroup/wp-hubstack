...
def get_default_log_config() -> Dict[str, Any]:
    """Get default log rotation configuration."""
    return {
...
def print_default_log_config_yaml() -> None:
    """Print the default log configuration in YAML format."""
    try:
        import yaml as pyyaml
    except ImportError:
        # Fallback if yaml is not available
        config = get_default_log_config()
        yaml_output = f"""# WordPress Update Suite Log Rotation Configuration
# This file allows you to customize log rotation settings
# Generated by: --print-rotate-log-yaml

# Log file path (default: /var/log/wp-update-suite.log)
log_file: {config['log_file']}

# Maximum file size before rotation in bytes (default: 10485760 = 10MB)
# You can use expressions like: 10 * 1024 * 1024 for 10MB
...
def setup_log_rotation(rotate_logs: bool = False, config_path: Optional[str] = None) -> None:
    """Setup log rotation with sensible defaults or custom YAML config."""
    if not rotate_logs:
        return
    ...
    if config_path and os.path.exists(config_path):
        try:
            if _yaml_type == 'ruamel' and RuamelYAML:
                yaml = RuamelYAML()
                custom_config = yaml.load(Path(config_path))
            elif _yaml_type == 'pyyaml' and pyyaml:
                with open(config_path, 'r') as f:
                    custom_config = pyyaml.safe_load(f)
            else:
                print("⚠️  No YAML parser available. Using default log rotation settings.")
                custom_config = {}
            
            if custom_config:
                log_config.update(custom_config)
                print(f"✅ Loaded custom log config from {config_path}")
        except Exception as e:
            print(f"⚠️  Error loading log config from {config_path}: {e}. Using defaults.")
    ...
    try:
        ...
        file_handler.setLevel(level_map.get(log_config['log_level'].upper(), logging.INFO))
        
        # Add handler to root logger
        root_logger = logging.getLogger()
        root_logger.addHandler(file_handler)
        
        # Keep console output for immediate feedback
        console_handler = logging.StreamHandler()
        console_handler.setFormatter(logging.Formatter(log_config['format']))
        root_logger.addHandler(console_handler)
        
        print(f"✅ Log rotation enabled. Logs will be written to {log_config['log_file']}")
        print(f"   Max file size: {log_config['max_bytes'] // (1024*1024)}MB, Backup count: {log_config['backup_count']}")
        
    except Exception as e:
        print(f"⚠️  Error setting up log rotation: {e}. Continuing with console logging only.")
...
def main():
    ...
    parser.add_argument('--print-rotate-log-yaml', action='store_true',
                       help='Print default log rotation YAML configuration and exit')

    args = parser.parse_args()
    
    # Handle print YAML config and exit
    if args.print_rotate_log_yaml:
        print_default_log_config_yaml()
        sys.exit(0)
    
    # Setup log rotation if requested
    setup_log_rotation(args.rotate_logs, args.log_config)
